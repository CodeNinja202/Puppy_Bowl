import { fetchAllPlayers, addNewPlayer,fetchSinglePlayer, removePlayer } from './ajaxHelpers';
...
export const renderAllPlayers = (playerList) => {
  ...
  for (let i = 0; i < detailButtons.length; i++) {
    ...
    button.addEventListener('click', async () => {
      const selectedPup = await fetchSinglePlayer(button.dataset.id);

      await renderSinglePlayer(selectedPup);
...
export const renderNewPlayerForm = () => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const puppyObj = {
      ...
      team: form.elements.team.name
    }
    
    await addNewPlayer(puppyObj);
    
    form.elements.name.value = '';
    form.elements.breed.value = '';
    form.elements.team.name = '';
    const puppyList = await fetchAllPlayers()
    renderAllPlayers(puppyList);
